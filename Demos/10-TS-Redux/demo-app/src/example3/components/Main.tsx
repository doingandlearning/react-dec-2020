...
function Main() {
    ...
    const [dummy, setDummy] : any = React.useState(undefined)

    React.useEffect(() => {
        console.log("In Example3 Main component, subscribing to Redux store")
        const unsubscribe = store.subscribe(() => {
            setDummy(new Date())  // Update component's dummy state, forces React to re-render component.
        })
        return () => {
            console.log("In Example3 Main component, unsubscribing from Redux store")
            unsubscribe()
        }
    }, [])
